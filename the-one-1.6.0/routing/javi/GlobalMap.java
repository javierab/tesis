...
public class GlobalMap {
	...
	public int addMap(GlobalMap otherMap){
		...
		if(mapExists && globalMap != null && synced == -1){
			localmix();
		}	
		/*if i cant make a map yet, i use the one i got*/
		else if(!mapExists || globalMap == null){
			int c = makeGlobal();
			if(c < 0){
		...
		if(mapExists && globalMap != null && (synced == 0 || synced == 1)){
			//core.Debug.p("nene");
	...
	public int makeGlobal(){
		if(mapExists && globalMap!= null && this.creationTime > SimClock.getTime() - TIMEOUT){
		//TODO: merge new maps	
	...
	private void localmix(){
		...
		for(Map.Entry<Integer, NodePositionsSet> local : this.allMaps.entrySet()){
			...
			//null map, omit it.
			if(currentlocal == null || globalMap == null){
				keysToRemove.add(local.getKey());
